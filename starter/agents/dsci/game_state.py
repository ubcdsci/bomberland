...
class GameState:
    ...
    def game_state_to_board_state(self) -> BoardState:
        ## TODO: this prolly makes more sense as the intial parse game state method
        ## and prolly makes more sense to convert all the methods here to work with
        ## the data in this format

        ## TODO: maybe it makes more sense to just work with maps from the moment
        ## of getting the data in the beginning
        
        ## TODO: we need to keep track of how many bombs our unit has out and the 
        ## starter code does nothing regarding that

        entities = []
        
        # table = {
        #     "m": Indestructible # metalBlock
        #     "w": Destructible # woodenBlock
        #     "o": Destructible # oreBlock
        #     "a": Ammo
        #     "bp":Radius
        #     "b": Bomb
        #     "x": Fire # both bomb fire and end game fire
        # }

        ## TODO: perhaps all these jump tables can be minimized by just having one here?

        for entity in self._state["entities"]:
            c = Coordinate(entity["x"], entity["y"])
            obj = None
            if entitiy["type"] == "m":
                #  Indestructible # metalBlock
                obj = Indestructible(c)
            elif entitiy["type"] == "o" or entitiy["type"] == "w":
                #  Destructible # oreBlock
                #  Destructible # woodenBlock
                obj = Destructible(c, entity["hp"])
            elif entitiy["type"] == "a":
                #  Ammo
                obj = Ammo(c, entity["hp"], entity["expires"])
            elif entitiy["type"] == "bp":
                # Radius
                obj = Radius(c, entity["hp"], entity["expires"])
            elif entitiy["type"] == "b":
                #  Bomb
                obj = Bomb(c, entity["owner_unit_id"], entity["expires"], entity["hp"], entity["blast_diameter"]
            elif entitiy["type"] == "x":
                #  Fire # both bomb fire and end game fire
                obj = Fire(c, entity["expires"], entity["owner_unit"])
            entities.append(obj)

        units = []
        ## a b ## these two are the agents
        ## c d ## and the 3 letters underneath are the units controlled by them
        ## e f ## which agent you are can be gotten from state.connection.agent_id
        ## g h
        aUnits = ["c", "e" ,"g"]
        bUnits = ["d", "f", "h"]
        for u in aUnits + bUnits:
            ## example below
            ## {'coordinates': [3, 10], 'hp': 3, 'inventory': {'bombs': 9999}, 'blast_diameter': 3, 'unit_id': 'c', 'agent_id': 'a', 'invulnerable': 0, 'stunned': 0}
            obj = UnitState(
                self._state["unit_state"][u]["unit_id"],
                self._state["unit_state"][u]["agent_id"],
                Coordinate(*self._state["unit_state"][u]["coordinates"]),
                self._state["unit_state"][u]["hp"],
                self._state["unit_state"][u]["inventory"]["bombs"],
                0,
                self._state["unit_state"][u]["blast_diameter"],
                self._state["unit_state"][u]["invulnerable"],
                ## TODO: find out what stunned does
                )
            units.append(obj)
        ...
        board = BoardState(
            entities,
            units,
            self._state["world"]["width"],
            self._state["world"]["height"],
            self._state["ticj"],
            )
    ...
    async def _on_game_tick(self, game_tick):
        ...
        ## TODO: turn self._state into a BoardState class here (???)
